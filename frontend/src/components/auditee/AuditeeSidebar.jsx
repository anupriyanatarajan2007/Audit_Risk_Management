...
const NAV_ITEMS = [
  { label: "Dashboard", icon: LayoutDashboard, path: "/auditee-officer" },
  { label: "My Audits", icon: ClipboardCheck, path: "/auditee-officer/assigned-audits" },
  { label: "Findings", icon: CircleAlert, path: "/auditee-officer/findings" },
  { label: "Submit Response", icon: MessageSquareText, path: "/auditee-officer/submit-response" },
  { label: "Evidence", icon: FolderUp, path: "/auditee-officer/upload-evidence" },
  { label: "Recommendations", icon: Lightbulb, path: "/auditee-officer/recommendations" },
];

const EASE = [0.4, 0, 0.2, 1];

export default function AuditeeSidebar({ collapsed, onToggleCollapse }) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        console.error("Failed to load Auditee profile:", error);
      }
    };
    fetchProfile();
  }, []);

  const firstName = profile?.firstName || "Auditee";
  ...
  return (
    ...
    >
        ...
        <AnimatePresence initial={false}>
          {!collapsed && (
            ...
            >
              ...
              <p className="text-slate-400 text-[11px] truncate mt-0.5">Auditee</p>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <style>
        {`
          .auditee-nav::-webkit-scrollbar { width: 6px; }
          .auditee-nav::-webkit-scrollbar-track { background: #050810; }
          .auditee-nav::-webkit-scrollbar-thumb { background: #111827; border-radius: 10px; }
          .auditee-nav:hover::-webkit-scrollbar-thumb { background: #374151; }
          .auditee-nav::-webkit-scrollbar-thumb:active { background: #10b981; }
          .auditee-nav { scrollbar-width: thin; scrollbar-color: #111827 #050810; }
          .auditee-nav:hover { scrollbar-color: #374151 #050810; }
        `}
      </style>

      {/* MAIN NAV — sidebar stays dark; only the hovered/active item shows accent */}
      <nav className="auditee-nav flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-3 py-4">
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.p
              initial={{ opacity: 0 }}
      ...
      <div className="shrink-0 p-3 border-t border-white/5 bg-[#0B1120] space-y-2">
        {[
          { label: "Notifications", icon: Bell, path: "/auditee-officer/notifications" },
          { label: "Profile", icon: UserCircle, path: "/auditee-officer/profile" },
