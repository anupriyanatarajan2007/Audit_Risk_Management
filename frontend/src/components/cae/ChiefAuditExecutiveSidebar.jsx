...
const NAV_ITEMS = [
  { label: "Dashboard", icon: LayoutDashboard, path: "/chief-audit-executive/dashboard" },
  { label: "Annual Audit Plan", icon: CalendarCheck, path: "/chief-audit-executive/annual-audit-plan" },
  { label: "Audit Portfolio", icon: BriefcaseBusiness, path: "/chief-audit-executive/audits" },
  { label: "Audit Monitoring", icon: Activity, path: "/chief-audit-executive/monitoring" },
  { label: "Risk Overview", icon: ShieldAlert, path: "/chief-audit-executive/risks" },
  { label: "Findings & Issues", icon: CircleAlert, path: "/chief-audit-executive/findings" },
  { label: "Recommendations", icon: Lightbulb, path: "/chief-audit-executive/recommendations" },
  { label: "Audit Reports", icon: FileBarChart, path: "/chief-audit-executive/reports" },
  { label: "Compliance Overview", icon: BadgeCheck, path: "/chief-audit-executive/compliance" },
  { label: "Audit Performance", icon: ChartNoAxesCombined, path: "/chief-audit-executive/performance" },
];

const EASE = [0.4, 0, 0.2, 1];

export default function ChiefAuditExecutiveSidebar({ collapsed, onToggleCollapse }) {
  ...
  const firstName = profile?.firstName || "Chief";
  ...
  return (
    ...
    >
        ...
        <AnimatePresence initial={false}>
          {!collapsed && (
            ...
            >
              ...
              {/* {/* <p className="text-white text-sm font-semibold truncate leading-tight">AUDIT & RISK</p> */}
              <p className="text-white text-sm font-semibold truncate leading-tight">{displayName}</p> 
              <p className="text-slate-400 text-[11px] truncate mt-0.5">Chief Audit Executive</p>
            </motion.div>
          )}
        </AnimatePresence>

        {/* CIRCLE TOGGLE — always visible, same as Audit Manager / Internal Auditor */}
        <button
          type="button"
      ...
      <style>
        {`
          .cae-nav::-webkit-scrollbar { width: 6px; }
          .cae-nav::-webkit-scrollbar-track { background: #050810; }
          .cae-nav::-webkit-scrollbar-thumb { background: #111827; border-radius: 10px; }
          .cae-nav:hover::-webkit-scrollbar-thumb { background: #374151; }
          .cae-nav::-webkit-scrollbar-thumb:active { background: #10b981; }
          .cae-nav { scrollbar-width: thin; scrollbar-color: #111827 #050810; }
          .cae-nav:hover { scrollbar-color: #374151 #050810; }
        `}
      </style>

      {/* MAIN NAV — sidebar stays dark; only the hovered/active item shows accent */}
      <nav className="cae-nav flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-3 py-4">
        <AnimatePresence initial={false}>
          {!collapsed && (
            <motion.p
              initial={{ opacity: 0 }}
      ...
      <div className="shrink-0 p-3 border-t border-white/5 bg-[#0B1120] space-y-2">
        {[
          { label: "Notifications", icon: Bell, path: "/chief-audit-executive/notifications" },
          { label: "Profile", icon: UserCircle, path: "/chief-audit-executive/profile" },
