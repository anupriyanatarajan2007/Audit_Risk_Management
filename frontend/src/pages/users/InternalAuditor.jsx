...
/* =========================================================
   ...
   This function also supports old string response:
...
const formatDepartment = (dept) => {
  ...
  // Backend returns department as an object
  if (typeof dept === "object") {
    return dept.name || "Unassigned";
  }

  // Backend returns department as a string
...
const formatRole = (role) => {
  ...
  // Backend returns role as object
...
export default function InternalAuditor() {
  ...
  const loadUsers = async () => {
    ...
    try {
      const response = await getUsersByRole("INTERNAL_AUDITOR");
      ...
      if (error?.response?.status === 403) {
        setError(
          "Access Denied - You don't have permission to view Internal Auditors"
        );
        ...
        setError(
          "Unable to load Internal Auditors"
        );
      }
    } finally {
      setLoading(false);
  ...
  /* IMPORTANT:
     department is an object, so we use department.id
